...
class KyuhouCsvValidator
  ...
  CASE    = {
    ...
    check_004: {
      :msg      => "提出義務者の住所又は所在地 全角60",
      :must     => true,
      :pattern  => /^[^ -~｡-ﾟ]+$/,
      :max_size => 60,
    },
    check_005: {
      :msg      => "提出義務者の氏名又は名称 全角30",
      :must     => true,
      :pattern  => /^[^ -~｡-ﾟ]+$/,
      :max_size => 30,
    },
  ...
  def validate(check_id, data, lineno)
    validate_result = OK
    error_msg = []
    if CASE[check_id][:must] && data.empty?
      validate_result = NG 
      error_msg.push "必須"
    end
    unless CASE[check_id][:pattern] =~ data.encode('utf-8')
      validate_result = NG
      error_msg.push "パターン"
    end
    if CASE[check_id][:max_size]
      if CASE[check_id][:max_size] < data.encode('utf-8').split(//).size
        validate_result = NG
        error_msg.push "サイズ"
      end
    end
    unless validate_result == OK
      logging(check_id, data, lineno, error_msg.join('_')) if @logging_to
      return NG
    end
    return OK
  end
  ...
  def logging(check_id, data, lineno, error_msg)
    @logging_to.write "#{sprintf('%05d', lineno)}|#{check_id.to_s}|".encode('cp932')
    @logging_to.write "#{CASE[check_id][:msg]}|#{error_msg}|".encode('cp932')
    if data == "\n"
      @logging_to.puts ">><<".encode('cp932')
    else
...
def main
  ...
  csv_files.each do |file|
    ...
    input.each_line.with_index do |line, lineno|
      ...
      field_data.each.with_index do |data, index|
